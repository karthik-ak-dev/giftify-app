...
import { UserProfile, UserProfileResponse, UpdateProfileRequest } from '../types/user';
...
export const updateUserProfile = async (data: UpdateProfileRequest): Promise<UserProfile> => {
  ...
  const response = await fetch(getApiUrl(API_CONFIG.ENDPOINTS.USERS.UPDATE_PROFILE), {
    method: 'PUT',
    headers: {
      'Authorization': `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
